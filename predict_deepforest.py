...
def process_all_tif_files_in_folder(model, folder_path, savedir, **kwargs):
    """
    Process only the merged DOP TIFF file (ending with 'dop_merged.tif') and save predictions as shapefiles.
    """
    # Adjust the folder_path to point to the 'output_wms' directory if it exists
    output_wms_path = os.path.join(folder_path, "output_wms")
    if os.path.exists(output_wms_path):
        folder_path = output_wms_path  # Update folder_path to point to 'output_wms'

    # Filter to include only the file ending with 'dop_merged.tif'
    tif_files = [f for f in os.listdir(folder_path) if f.endswith('dop_merged.tif')]

    # Process the merged DOP file if it exists
    for tif_file in tif_files:
        image_path = os.path.join(folder_path, tif_file)
        print(f"Processing merged DOP file: {image_path}...")

        # Call the prediction and saving function for the merged file
